...
from common.feature_spec import QUERIED_FEATURES, TEMPORAL_FEATURES
from config import PROMETHEUS_URL, QUERIES, TARGET_APP

print(f"\n{'=' * 55}")
print(f"Feature Verification — {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}")
...
print(f"\n{'=' * 55}")
print("All required queried features are present." if all_good else "Some features are missing; do not export training data yet.")
print(f"{'=' * 55}\n")
