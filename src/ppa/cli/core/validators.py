...
def validate_app_name(name: str) -> str:
    ...
    if not APP_NAME_PATTERN.match(name):
        raise ValidationError(
            ...
            suggestion="Use lowercase alphanumerics and hyphens only (not starting/ending with hyphen)",
        )
    ...
    try:
        ...
        if result.returncode != 0:
            raise KubernetesError(
                ...
                suggestion="Check kubeconfig (export KUBECONFIG=/path/to/config) or start a cluster (minikube start)",
            )
...
def validate_prometheus_connection(url: str) -> None:
    ...
    try:
        ...
        raise PrometheusError(
            ...
            context={"error": str(e)},
            suggestion="Ensure Prometheus is running and accessible (check port forwarding if remote)",
        ) from e
