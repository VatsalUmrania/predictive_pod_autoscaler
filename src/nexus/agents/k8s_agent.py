    ...
    def _check_pod(self, pod) -> list[IncidentEvent]:
            ...
            if key not in self._pending_since:
                ...
                events.append(
                    IncidentEvent(
                        ...
                        context={
                            "pod_name": name,
                            "pending_minutes": (now - self._pending_since[key]) / 60.0,
                        },
                    )
                )
        else:
        ...
        for cs in pod.status.container_statuses or []:
            ...
            if (
                ...
                and restart_count >= self.crashloop_threshold
            ):
                self._emitted_this_cycle.add(key)
    ...
    async def sense(self) -> list[IncidentEvent]:
        ...
        if self.namespaces:
            ...
            result = await loop.run_in_executor(
                None, self._k8s_core.list_pod_for_all_namespaces
            )
            pods = result.items
        ...
        if self.namespaces:
            dep_lists = await asyncio.gather(
                *[
                    loop.run_in_executor(
                        None, self._k8s_apps.list_namespaced_deployment, ns
                    )
                    for ns in self.namespaces
                ],
                return_exceptions=True,
            ...
            result = await loop.run_in_executor(
                None, self._k8s_apps.list_deployment_for_all_namespaces
            )
            ...
            if self.namespaces:
                hpa_lists = await asyncio.gather(
                    *[
                        loop.run_in_executor(
                            None,
                            self._k8s_autoscaling.list_namespaced_horizontal_pod_autoscaler,
                            ns,
                        )
                        for ns in self.namespaces
                    ],
                ...
                result = await loop.run_in_executor(
                    None,
                    self._k8s_autoscaling.list_horizontal_pod_autoscaler_for_all_namespaces,
                )
                hpas = result.items
