...
from ppa.model.pipeline import should_promote  # noqa: E402
...
class TestPipelineOrchestrator:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = subprocess.run(
                [
                    ...
                    tmpdir,
                    "--quality-gate",
                    "100.0",  # High threshold for synthetic data
                ],
                capture_output=True,
                text=True,
                timeout=120,
            ...
            # Verify artifacts (canonical paths: no horizon suffix in filenames)
            horizon = "rps_t3m"
            assert Path(f"{tmpdir}/ppa_model.keras").exists()
            assert Path(f"{tmpdir}/ppa_model.tflite").exists()
            assert Path(f"{tmpdir}/scaler.pkl").exists()
            assert Path(f"{tmpdir}/target_scaler.pkl").exists()
    ...
    def test_pipeline_multiple_horizons(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Verify all horizons have artifacts (canonical paths: no horizon suffix in filenames)
