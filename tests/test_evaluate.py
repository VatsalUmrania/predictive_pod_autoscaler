...
import ppa.model.evaluate as evaluate_module  # noqa: E402
from ppa.common.feature_spec import FEATURE_COLUMNS, TARGET_COLUMNS  # noqa: E402
from ppa.model.evaluate import (  # noqa: E402
    compute_mae,
    compute_mape,
    compute_rmse,
...
from ppa.model.train import train_model  # noqa: E402
...
class TestEvaluateModel:
    ...
    def test_evaluation_skips_plots_when_matplotlib_missing(self, monkeypatch):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for path in [model_path, scaler_path, csv_path]:
                Path(path).touch()
            ...
            class DummyModel:
                def predict(self, x, verbose=0):
                    return np.full((len(x), 1), 12.0)

            monkeypatch.setattr(
                evaluate_module._keras.models,
                "load_model",
            ...
            monkeypatch.setattr(
                ...
                lambda df, features, target_col, scaler, lookback: (
                    np.zeros((8, 2, len(FEATURE_COLUMNS))),
                    np.full(8, 10.0),
                ),
            )
            monkeypatch.setattr(evaluate_module, "_load_pyplot", lambda: None)
