        ...
        "spec": {
            ...
            "template": {
                "spec": {
                    ...
                    "containers": [{
                        "name": "retraining",
                        "image": os.getenv("RETRAINING_IMAGE", "ppa-ml-pipeline:latest"),
                        "command": ["python", "-m", "model.train"],
                        "args": [
                            "--app", cr_info["cr_name"],
                            "--horizon", "rps_t3m",
                            "--evaluate-before-promote", "true",
...
def main():
    ...
    logger.info(f"Starting retraining controller (threshold: {DRIFT_THRESHOLD}%, duration: {DRIFT_DURATION_MINUTES}min)")
    ...
    for cr in crs_to_retrain:
        logger.info(f"Triggering retraining for {cr['cr_name']} (error: {cr['error_pct']:.1f}%)")
        if create_retraining_job(cr):
            success_count += 1

    logger.info(f"Successfully triggered {success_count}/{len(crs_to_retrain)} retraining jobs")
