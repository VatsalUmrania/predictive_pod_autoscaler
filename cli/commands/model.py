...
from cli.utils import console, error, heading, info, step_heading, success, warn
...
def model_train(
    ...
    target: str = typer.Option(DEFAULT_HORIZON, "--target", help="Target column (rps_t3m, rps_t5m, rps_t10m)."),
    ...
    output_dir: str = typer.Option(str(ARTIFACTS_DIR), "--output-dir", help="Output directory for artifacts."),
    ...
    for k, v in {"Target": target, "CSV": csv, "Epochs": str(epochs),
                  "Lookback": str(lookback), "Patience": str(patience)}.items():
        config.add_row(k, v)
    ...
    try:
        ...
        results_table = Table(title="[bold]Training Results[/]", border_style="green", header_style="bold")
...
def model_evaluate(
    ...
    target_scaler: str | None = typer.Option(None, "--target-scaler", help="Path to target scaler .pkl."),
    test_split: float = typer.Option(0.1, "--test-split", help="Test set fraction."),
    low_traffic: float = typer.Option(10.0, "--low-traffic-threshold", help="Min RPS for filtered MAPE."),
) -> None:
    """
    ...
    try:
        ...
        # Results table
        table = Table(title="[bold]Evaluation Results[/]", border_style="green", header_style="bold")
        ...
        cmp = Table(title="[bold]PPA vs HPA Comparison[/]", border_style="bright_cyan", header_style="bold")
        cmp.add_column("Metric", style="bold", min_width=24)
        cmp.add_column("PPA", justify="right")
        cmp.add_column("HPA", justify="right")

        cmp.add_row("Avg Replicas", f"{result.get('ppa_avg_replicas', 0):.2f}", f"{result.get('hpa_avg_replicas', 0):.2f}")
        cmp.add_row("Over-provisioned %", f"{result.get('ppa_over_prov_pct', 0):.1f}", f"{result.get('hpa_over_prov_pct', 0):.1f}")
        cmp.add_row("Under-provisioned %", f"{result.get('ppa_under_prov_pct', 0):.1f}", f"{result.get('hpa_under_prov_pct', 0):.1f}")
        cmp.add_row("Replica savings", f"[green]{result.get('replica_savings_pct', 0):.1f}%[/green]", "—")
...
def model_pipeline(
    ...
    csv: str = typer.Option(DEFAULT_CSV, "--csv", help="Path to training CSV."),
    horizons: str = typer.Option("rps_t3m,rps_t5m,rps_t10m", "--horizons", help="Comma-separated targets."),
    ...
    patience: int = typer.Option(15, "--patience", help="Early stopping patience."),
    promote: bool = typer.Option(False, "--promote-if-better", help="Auto-promote if challenger beats champion."),
) -> None:
    """
    ...
    try:
        ...
        if exit_code == 0:
            console.print(Panel("[success]Pipeline complete — all horizons passed[/success]", border_style="green"))
        else:
            console.print(Panel("[warning]Pipeline complete — some horizons failed quality gate[/warning]", border_style="yellow"))
