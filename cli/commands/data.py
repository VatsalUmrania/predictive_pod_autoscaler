...
from cli.config import DEFAULT_APP_NAME, DEFAULT_CSV, PROJECT_DIR, TRAINING_DATA_DIR
from cli.utils import console, error, heading, info, success, warn
...
def data_export(
    ...
    try:
        from data_collection_module import export_training_data
    except ImportError:
        pass

    try:
        # Import directly from the project
        sys.path.insert(0, str(PROJECT_DIR / "data-collection"))
        from export_training_data import build_feature_dataframe, build_dataset_health, print_dataset_health, write_health_report, add_segment_ids
...
def data_health(
    ...
    if os.path.exists(health_json):
        ...
        try:
            import pandas as pd
            from export_training_data import build_dataset_health
